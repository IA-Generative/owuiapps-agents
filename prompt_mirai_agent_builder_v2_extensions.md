...
| **P13** | **Exécution sandboxée côté serveur** | OpenAI Code Interpreter (sandbox Python), Anthropic Claude analysis tool | L'agent peut exécuter du code Python/JS dans un conteneur éphémère isolé (pas sur le poste utilisateur). Pour : calculs, transformations de données, génération de graphiques. | OpenWebUI a déjà un "code execution" basique. L'améliorer avec des images sandboxées + accès lecture aux collections. |
